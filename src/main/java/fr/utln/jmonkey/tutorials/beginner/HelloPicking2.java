...
public class HelloPicking2 extends SimpleApplication
{

  public static void main(String[] args)
  {
    HelloPicking2 app = new HelloPicking2();
    app.start();
  }
  ...
  public void simpleInitApp()
  {
    ...
    rootNode.attachChild(inventory);
    // add a light to the HUD so we can see the robot
    DirectionalLight sun = new DirectionalLight();
    sun.setDirection(new Vector3f(0, -1.0f, 0));
    ...
    inputManager.addListener(new AnalogListener() {
      ...
      public void onAnalog(String name, float value, float tpf) {
        ...
        if (results.size() > 0)
        {
          ...
          lastCorrectPos = closest.getContactPoint().clone();
          item.setLocalTranslation(closest.getContactPoint().clone());
        }
    ...
    public void onAction(String name, boolean keyPressed, float tpf)
    {
      if (name.equals("Shoot") && !keyPressed)
      {
        if (!inventory.getChildren().isEmpty())
        {
          ...
          s1.setLocalTranslation(lastCorrectPos);
